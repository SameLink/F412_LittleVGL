...
#define LCD_D4            B, 5
#define LCD_D5            B, 4
#define LCD_D6            B, 10 // TS_XP
#define LCD_D7            A, 8  // TS_YM
...
In the lcd board are parallel pins
 - TS_XM <- LCD_RS
 - TS_XP <- LCD_D6
 - TS_YM <- LCD_D7
 - TS_YP <- LCD_WR */
...
/* Wait time for LCD write and read pulse and touchscreen AD converter
   - First, give 10, 20, 500 values, then lower them to speed up the program.
     (values also depend on processor speed and LCD speed) */
#define LCD_WRITE_DELAY  0
#define LCD_READ_DELAY   0
#define TS_AD_DELAY      0
